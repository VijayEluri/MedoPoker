...
public class PokerCanvas extends GameCanvas {
    ...
    private final boolean BETTER_GRAPHICS;
    ...
    public PokerCanvas(ClientManager cm, boolean graphics) {
        super(false);
        this.cm = cm;
        this.setFullScreenMode(true);

        BETTER_GRAPHICS = graphics;
    ...
    private void paintMiddle() {
        ...
        g.setFont(font_pot);
        if (pot != null) {
            g.drawString(pot, screen_width / 2, screen_height / 2 + 5, Graphics.BASELINE | Graphics.HCENTER);
        }
        g.setFont(Font.getFont(Font.FACE_PROPORTIONAL, Font.STYLE_PLAIN, Font.SIZE_SMALL));
        g.setColor(0, 0, 0);
        if (highest_bet != null) {
            g.drawString(highest_bet, screen_width / 2, screen_height / 2 + 10 + font_pot.getHeight(), Graphics.BASELINE | Graphics.HCENTER);
        }
        ...
        if (current_player != -1) {
            paintPlayerAction(current_action, current_player);
        }
    }
    ...
    private void createBG() {
        ...
        if (BETTER_GRAPHICS) {
            ...
            bg_g.setColor(110, 215, 40);
            bg_g.fillRect(0, 0, screen_width, screen_height);
        }
    ...
    private void loadImages() throws IOException {
        ...
        if (BETTER_GRAPHICS) {
            bg_pattern = Image.createImage(img_path+"stripe.png");
        }
    }
